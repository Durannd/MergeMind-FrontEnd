...
function LoginPage() {
  ...
  const { user, login, isLoginPending, errorLogin } = useContext(AuthContext);
  ...
  const canSubmit = isEmailValid && isPasswordValid && !isLoginPending;
    ...
    <div className="flex flex-row w-full h-screen">
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center bg-[#0d1117] text-white">
        <form onSubmit={handleLogin} className="flex w-full max-w-md md:max-w-lg flex-col gap-4 p-8 bg-gray-900 rounded-lg shadow-xl border border-gray-800">
          ...
          {errorLogin && <p className="text-red-500 mt-2">{errorLogin.message}</p>}
        </form>
      </div>
        ...
        <div className="h-96 sm:h-[450px] xl:h-[550px] 2xl:h-[650px] w-full max-w-2xl relative rounded-2xl">
          <Carousel slideInterval={5000} indicators={false} >
            <div className="relative h-full w-full">
              <img src={photo1} alt="Slide 1" className="absolute inset-0 w-full h-full object-cover " />
            </div>
            <div className="relative h-full w-full">
              <img src={photo2} alt="Slide 2" className="absolute inset-0 w-full h-full object-cover " />
            </div>
            <div className="relative h-full w-full">
              <img src={photo3} alt="Slide 3" className="absolute inset-0 w-full h-full object-cover " />
            </div>
          </Carousel>
        </div>
      </div>
